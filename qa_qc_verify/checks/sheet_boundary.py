...
def verify_sheet_boundaries(job: Job, report: QAReport, config: Config) -> QAReport:
 ...
 margin_inches
        ...
        for handle in detail_sheet_handles:
            try:
                ...
                sheet_info = SheetInfo(
                    ...
                    rotation=math.degrees(status.DetailRotation) if status and status.DetailRotation else 0,
                    revision=str(status.SheetRevision) if status and status.SheetRevision else None,
                    detail_count=0,
                ...
                margin_units = margin_inches * status.DrawingScale if status else margin_inches
                ...
                for detail_handle in detail_handles:
                    try:
                        ...
                        sheet_list = detail_status.Sheet.split(',') if detail_status.Sheet else ""
                        ...
                        if viewport_length_scaled > effective_length or viewport_height_scaled > effective_height:
                            issue = {
    ...
    try:
        ...
        for handle in erection_sheet_handles:
            try:
                ...
                erection_view_handles = job.GetDrawingHandles(TableWithDrawings.ErectionView)
                for view_handle in erection_view_handles:
                    try:
                        ...
                        sheet_list = view_status.Sheet.split(',') if view_status.Sheet else ""
                        if drawing.Name not in sheet_list:
                            continue
                        ...
                        if view_length_scaled > effective_length or view_height_scaled > effective_height:
                            issue = {
                                'type': 'view_overlap',
    ...
    # Summary
    detail_sheets_with_issues = sum(1 for s in sheets_data if s.issues and s.drawing_type == "DetailSheet")
    erection_sheets_with_issues = sum(1 for s in sheets_data if s.issues and s.drawing_type == "ErectionSheet")
