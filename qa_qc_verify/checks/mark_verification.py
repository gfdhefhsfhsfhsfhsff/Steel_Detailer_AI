...
try:
    from DesignData.SDS2.Database import (
        Job, MemberHandle, MemberHandleList, DrawingHandle, DrawingHandleList,
        TableWithDrawings, ReadOnlyTransaction
    )
    from DesignData.SDS2.Model import MemberBrief, Member
    from DesignData.SDS2.Detail import Drawing, DrawingList, DrawingStatus, PiecemarkType
    API_AVAILABLE = True
except ImportError:
    API_AVAILABLE = False
...
class MemberInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'dates': {
                'model_complete_date': str(self.model_complete_date) if self.model_complete_date else None,
                'fabrication_projected_date': str(self.fabrication_projected_date) if self.fabrication_projected_date else None,
                'fabrication_shop_date': str(self.fabrication_shop_date) if self.fabrication_shop_date else None,
                'fabrication_complete_date': str(self.fabrication_complete_date) if self.fabrication_complete_date else None,
                'ship_date': str(self.ship_date) if self.ship_date else None,
                'projected_ship_date': str(self.projected_ship_date) if self.projected_ship_date else None,
                'received_on_job_site': str(self.received_on_job_site) if self.received_on_job_site else None,
                'erected_date': str(self.erected_date) if self.erected_date else None,
                'submitted_for_approval': str(self.submitted_for_approval) if self.submitted_for_approval else None,
                'received_for_approval': str(self.received_for_approval) if self.received_for_approval else None,
            },
            'other': {
                'description': self.description,
                'swap_ends': self.swap_ends,
                'model_complete_mode': str(self.model_complete_mode) if self.model_complete_mode else None,
                ...
                'custom_property_handle': str(self.custom_property_handle) if self.custom_property_handle else None,
                'guid': str(self.guid) if self.guid else None,
                'to_global_coordinates': str(self.to_global_coordinates) if self.to_global_coordinates else None
            }
        }
    ...
    for i, range(total_members):
        ...
        try:
            ...
            member_info = MemberInfo(
                ...
                member_type=str(member_brief.MemberType) if member_brief.MemberType else "Unknown",
                ...
                surface_finish=str(member_brief.SurfaceFinish) if member_brief.Surface_finish else None,
                ...
                model_complete_date=str(member_brief.ModelCompleteDate) if member_brief.ModelCompleteDate else None,
                model_complete_mode=str(member_brief.ModelCompleteMode) if member_brief.ModelCompleteMode else None,
                fabrication_projected_date=str(member_brief.FabricationProjectedDate) if member_brief.FabricationProjectedDate else None,
                fabrication_shop_date=str(member_brief.FabricationShopDate) if member_brief.FabricationShopDate else None,
                fabrication_complete_date=str(member_brief.FabricationCompleteDate) if member_brief.FabricationCompleteDate else None,
                ship_date=str(member_brief.ShipDate) if member_brief.ShipDate else None,
                projected_ship_date=str(member_brief.ProjectedShipDate) if member_brief.ProjectedShipDate else None,
                received_on_job_site=str(member_brief.ReceivedOnJobSite) if member_brief.ReceivedOnJobSite else None,
                erected_date=str(member_brief.Erected) if member_brief.Erected else None,
                submitted_for_approval=str(member_brief.SubmittedForApproval) if member_brief.SubmittedForApproval else None,
                received_for_approval=str(member_brief.ReceivedForApproval) if member_brief.ReceivedForApproval else None,
            ...
            try:
                ...
                if drawing_list and len(drawing_list) > 0:
                    ...
                    member_info.sheets = drawing.Status.Sheet.split(',') if drawing.Status.Sheet else []
                    member_info.revision = str(drawing.Status.SheetRevision) if drawing.Status.SheetRevision else None
            except Exception:
                pass
            ...
            report.add_warning(
                module="Mark",
                object_type="Member",
                location=f"Member #{member_info.number}",
